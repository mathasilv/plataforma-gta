...
export function CarregadorParamsForm({ onSaved }: { onSaved?: (p: Params) => void }) {
  ...
  useEffect(() => {
    fetch("/api/carregador/config").then((r) => r.json()).then((d) => {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      const res = await fetch("/api/carregador/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(valores) });
